function initGraph(customColorStr, graphId, initData, horDist, verDist) {
  const customColor = am4core.color(customColorStr);
  // Themes begin
  am4core.useTheme(am4themes_dark);
  am4core.useTheme(am4themes_animated);
  // Themes end

  const chart = am4core.create(graphId, am4charts.XYChart);
  ...
  const line = new am4charts.LineSeries();
  line.fill = customColor;

  const series = chart.series.push(line);
  ...
  // bullet at the front of the line
  const bullet1 = series.bullets.push(new am4charts.CircleBullet());
